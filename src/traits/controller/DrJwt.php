...
/**
 * author      : Qttx 摩羯Ж'
 * mtime       : 2020/1/8 10:04
 * description : 简易jwt trait,只做加密和过期验证
 */

namespace dr\traits\controller;

trait DrJwt
{

    private $alg = 'sha256';
    private $secret = "38726";
    ...
    public static function token($uid) {
        return (new self())->genToken($uid);
    }

    // 静态方式调用验证token
    public static function authToken($token) {
        return (new self())->verifyToken($token);
    }
     ...
     * JWT 规定了7个官方字段，这里只关注过期时间和uid
     * @param $uid int 用户id
     * @return mixed
     */
    protected function getPayload($uid) {
        $now     = time();
        $payload = [
            'iss' => 'admin', //签发人
            'exp' => $now + 600, //过期时间
